...
*/
{
  ...
  outputTourFile( best_so_far_ant->tour );
...
void outputTourFile( long int *t ) 
/*    
      FUNCTION:       print the tour *t to outputfile
      INPUT:          pointer to a tour
      OUTPUT:         none
*/
{
	FILE *output_file = fopen(output_name_buf, "w");

    long int   i;
    if (!output_file) return;

    fprintf(output_file,"Tour length %ld\n",compute_tour_length( t ));
    for( i = 0 ; i < n ; i++ ) {
	fprintf(output_file,"%ld ", t[i]);
    }
