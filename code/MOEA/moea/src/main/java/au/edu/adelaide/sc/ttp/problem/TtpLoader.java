...
public class TtpLoader {
	...
	public static Ttp readProblemHead(Scanner dataFile, String problemFileName) {
		...
		return new Ttp(problemName, problemFileName, noOfNodes, noOfItems, rentingRate.doubleValue(),
				minSpeed.doubleValue(), maxSpeed.doubleValue(), capacity, weightType, null, null);
	...
	public static Ttp loadProblem(String problemFileName) {
		...
		try (Scanner dataFile = new Scanner(new File(problemFileName))) {
			...
			problem = readProblemHead(dataFile, problemFileName);
			...
			if (problem == null || problem.getNoOfNodes() != cities.size() || problem.getNoOfItems() != items.size()) {
			...
			problem = new Ttp(problem.name, problem.path, problem.n, problem.m, problem.rentingRate, problem.minSpeed,
					problem.maxSpeed, problem.capacity, problem.edgeWeightType, cities, items);
